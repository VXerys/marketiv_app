...
| **Pekerjaan Aktif & Submit Bukti** | ✅ Selesai | Implementasi lengkap (Data, Domain, Presentation, UI). Mencakup `PekerjaanEntity` dengan computed getters (`isSubmitted`, `isDone`, `isFraud`, `isPending`, `estimasiPendapatan`), `PekerjaanAktifRemoteDataSource` dengan parallel fetch campaign+submission via `Future.wait`, `PekerjaanAktifController` dengan filter status dan validasi URL (TikTok/IG), `PekerjaanAktifPage` (list dengan FilterChip per status), serta `SubmitBuktiPage` (form submit + panduan langkah + bottom bar dinamis via `bottomNavigationBar` Scaffold). |
...
- **Implementasi & Review Pekerjaan Aktif + Submit Bukti Tayang (Kreator)** — Modul `pekerjaan_aktif` selesai diimplementasikan end-to-end. Hasil review statik: (1) Fix constructor `PekerjaanEntity` — nullable fields `niche` & `urlBuktiTayang` tidak lagi `required`; (2) Hapus `toEntity()` yang redundant di `PekerjaanModel` karena Model sudah IS-A Entity; (3) Update `RepositoryImpl` agar tidak memanggil `toEntity()`; (4) Fix `_PekerjaanCard` — hapus `SizedBox(height: 145)` fixed yang menyebabkan overflow saat tombol "Submit Bukti ->" ditampilkan. Seluruh checklist routing, binding, null safety, dan logic lulus review.
