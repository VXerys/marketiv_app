...
```
1. User isi form: nama_lengkap, email, password, nomor_whatsapp
2. User WAJIB pilih role: "Saya Pemilik UMKM" atau "Saya Konten Kreator"
3. AuthController → RegisterUseCase → AuthRepository → Appwrite Account.create()
4. Appwrite kirim email verifikasi (Account.createVerification())
5. Setelah verifikasi → is_verified = TRUE di collection users
6. Profile document otomatis dibuat di collection `users` setelah registrasi sukses
