...
- Gunakan pagination: `Query.limit()` + `Query.offset()` untuk list panjang
- Debounce pencarian sekitar 300ms sebelum trigger API
- Gunakan `Query.select([...])` untuk ambil field yang diperlukan saja
- Kompres gambar sebelum upload (flutter_image_compress)
...
- Default: gunakan agent `marketiv-orchestrator` untuk semua prompt jika user tidak menyebut agent spesifik.
- Jika user menyebut agent spesifik, gunakan agent tersebut.
- Mapping kata kunci → agent:
  - UI, widget, page, layout, styling, design token → `marketiv-ui`
  - Appwrite, DataSource, permissions, query, storage, functions, realtime → `marketiv-appwrite`
  - login, register, logout, session, RBAC, role → `marketiv-auth`
  - campaign, job pool, submission, claim, bukti tayang → `marketiv-campaign`
  - rate card, chat, custom offer, order, negosiasi → `marketiv-ratecard`
...
- Jika request lintas domain, orchestrator memecah tugas atau memulai dari dependency paling dasar.
- Jika masih ambigu, fallback ke `marketiv-senior-flutter`.
...
- Validasi URL eksternal (Drive/Dropbox/TikTok/IG) di sisi Flutter sebelum disimpan

---

## Biaya & Skala MVP

- Utamakan Free Tier (Appwrite Functions) dan hindari server berbayar di fase MVP
- Jangan simpan video mentah di Storage; wajib gunakan URL eksternal untuk file besar
